...
const SignUp = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/item/signup', {
        method: 'POST',
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-[#FF9933] via-white to-[#138808]">
      <div className="bg-white p-8 rounded-lg shadow-xl w-96 border-4 border-[#FF9933]">
        <h2 className="text-3xl font-extrabold mb-6 text-center text-[#138808]">Sign Up</h2>
        
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="username" className="block text-sm font-medium text-[#FF9933]">
              Username
            </label>
            <input
              type="text"
        ...
        <p className="mt-4 text-center text-sm">
          ...
          <span className="text-[#FF9933] font-bold cursor-pointer hover:underline" onClick={handleLogin}>
            Login here
          </span>
        </p>
      </div>
